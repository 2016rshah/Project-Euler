...
public class ProjE11{
	public static void main(String[] args){
		int[][] grid = makeGrid();
		long[] results = {maxHorizontal(4, grid), maxVertical(4, grid), maxDiagonal(4, grid), maxDiagonalRL(4, grid)};
		Arrays.sort(results);
		System.out.println("Answer: "+results[results.length-1]);
	}
	public static long maxHorizontal(int len, int[][] grid){
		long result = 1L;
	...
	public static long maxVertical(int len, int[][] grid){
		long result = 1L;
	...
	public static long maxDiagonal(int len, int[][] grid){
		long maxProduct = 1L;
		for(int i = 0; i<grid.length-len+1; i++){
	...
	public static long maxDiagonalRL(int len, int[][] grid){ //There is a second set of diagonals from right to left
		long maxProduct = 1L;
		for(int i = 0; i<grid.length-len+1; i++){
			for(int j = grid[0].length-1; j>=len; j--){
				long product = 1L;
				for(int a = 0; a<len; a++)
					product*=grid[i+a][j-a];
				if(product>maxProduct)
